...
The project exposes Python bindings and can also expose reusable Rust code. Cargo package names normally use hyphens, while Rust crate identifiers use underscores. The binding crate and core crate also have different publication boundaries. The wheel ABI determines how many Python-specific wheels the release must build.
...
- the binding module is `robust_maturin_demo._native` and its public exports have matching stubs;
